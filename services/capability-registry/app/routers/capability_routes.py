...
def _validate_pack_shape_like_spec(pack: CapabilityPack):
    ...
    Allows reads to be lenient so legacy or transient states don't crash.
    """
    ...
    capset = set(pack.capability_ids)
    for pb in pack.playbooks:
        ...
        for st in pb.steps:
            if st.capability_id not in capset:
                raise HTTPException(
                    status_code=422,
                    detail=f"Playbook step '{st.id}' references unknown capability_id '{st.capability_id}'",
                )
...
async def reorder_steps(
    ...
    for pb in pack.playbooks:
        ...
        new_pbs.append(Playbook(id=pb.id, name=pb.name, description=pb.description, steps=reordered))

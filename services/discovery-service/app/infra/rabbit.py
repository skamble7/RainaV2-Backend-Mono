...
logger = logging.getLogger("discovery.events.rabbit")
...
def publish_event_v1(
    *,
    ...
    """
    Publish a versioned discovery event:
        <org>.discovery.<event>.<version>

    Example:
        publish_event_v1(org="raina", event="completed", payload={"workspace_id": "...", ...})

    Returns True on success; retries once after reconnect on failure.
    """
    routing_key = rk(org=org, service=Service.DISCOVERY, event=event, version=version)
    ...
    for attempt in (1, 2):
        try:
            ...
            logger.exception(
                "Rabbit publish failed; %s", type(e).__name__,
...
# Hard deprecation of the old shape to avoid accidental use
def publish_event(*args, **kwargs):  # pragma: no cover
    raise RuntimeError(
        "publish_event(event_type, workspace_id, payload) is removed. "
        "Use publish_event_v1(org, event, payload) with versioned routing keys."
    )

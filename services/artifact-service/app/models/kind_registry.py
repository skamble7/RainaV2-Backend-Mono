...
class PromptVariantSpec(BaseModel):
    name: str
    when: Optional[Dict[str, Any]] = None  # e.g. {"paradigm": "service-based", "style": "microservices"}
...
class IdentitySpec(BaseModel):
    ...
    natural_key: Optional[Any] = None  # e.g., ["data.service.name", "data.style"] OR JSONPath/JMESPath string
    summary_rule: Optional[str] = None  # e.g., "{{data.service.name}} API ({{data.style}})"
    category: Optional[str] = None      # often fixed by kind


class AdapterSpec(BaseModel):
    # Adapters normalize model/legacy output → canonical "data"
    # Either reference a builtin by id, or a safe declarative mapping (DSL)
...
class KindRegistryDoc(BaseModel):
    id: str = Field(alias="_id")               # canonical kind id, e.g. "cam.contract.api"

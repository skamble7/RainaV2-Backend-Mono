...
from app.services.registry_service import KindRegistryService
...
async def api_get_prompt(
    ...
    # any other selector passthrough
    db: AsyncIOMotorDatabase = Depends(get_db),
):
...
async def api_adapt_sample(
    ...
    """
    Given draft 'data' (payload), apply registry adapters to return the canonicalized form.
    """
...
# ─────────────────────────────────────────────────────────────
# Admin APIs (guard via a proxy / auth layer in deployment)
# ─────────────────────────────────────────────────────────────
@router.post("/kinds")
async def api_upsert_kind(
    ...
    """
    Admin-only: Insert/replace a kind definition.
    """
    if "_id" not in body:

...
# ─────────────────────────────────────────────────────────────
# NEW: bulk fetch by keys
# ─────────────────────────────────────────────────────────────
class KeysIn(BaseModel):
    keys: List[str] = Field(..., min_items=1, description="List of category keys, e.g., ['diagram','pat','dam']")

@router.post("/by-keys", response_model=list[CategoryDoc])
async def categories_by_keys(body: KeysIn):
    """
    Return categories for the provided keys.
    - Order matches the request keys.
